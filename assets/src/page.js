        ...
        <div className="flex flex-wrap items-center justify-center h-full w-full mb-20 flex-col items-center text-center">
          <h1 className="md:text-3xl text-2xl font-medium title-font mb-2 text-gray-900">{pages.TITLE}</h1>
          {
            ...
            Object.keys(textes).map((key, index)=>{
              if(textes){
              return(
                <div key={index} className="flex flex-col shadow-md md:shadow-none rounded-lg md:rounded-none my-5 overflow-hidden md:mb-0 md:mx-auto md:flex md:items-center md:justify-center md:relative md:w-full md:float-left md:max-w-4xl">
                  <div className="md:float-right md:relative md:px-3 max-w-xl">
                    <img src={textes[key].photo} />
                  </div>
